...
class GasStationSystem(BaseSystem):
    ...
    def get_transactions(self,
        ...
        response = self.connection.get(f'{credential.url}/account/transactions?page_size=100&page=1',
                                       headers=headers)
        transactions = []
        page = 1
        while True:
            response = requests.get(f'{credential.url}/account/transactions?page_size=100&page={page}',
                                    headers=headers)
            if response.status_code != 200:
                break
            soup = BeautifulSoup(response.text, 'lxml')
            page_transactions = extract_transactions(soup,
                                                     credential,
                                                     params_date)
